import '../../src/index.css';
import React from 'react';

function Footer() {
    return (
        <div className="w-full bg-black font-Montserrat font-normal relative">
            <div className="relative w-full mx-auto min-h-[550px] p-4 md:p-9 overflow-hidden">
                {/* Background Video */}
                <video
                    autoPlay
                    loop
                    muted
                    playsInline
                    className="absolute top-0 left-0 w-full h-full object-cover z-0 filter brightness-70"
                >
                    <source src="/footer.mp4" type="video/mp4" />
                    Your browser does not support the video tag.
                </video>

                {/* Overlay */}
                <div className="absolute top-0 left-0 w-full h-full bg-black opacity-30 z-10" />

                {/* Text content */}
                <div className="relative z-20 text-white grid grid-rows-2 gap-8">
                    {/* Content for c-1 */}
                    <div className="c-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 sm:gap-4 text-center sm:text-left">
                        {/* HQ Office */}
                        <div className="r1">
                            <h1 className="text-xl text-black">HQ Office</h1>
                            <p className='text-md'>via di villamagna 98</p>
                            <p className='text-md'>Firenze, Italy 50126</p> <br />
                            <p className='text-md'>Weekday: 9:00 AM to 5:00 PM</p> <br />
                            <h1 className="text-xl text-black">Phone number</h1>
                            <p className='text-md'>+393318476757</p>
                            <p className='text-md'>+393249887245</p>
                        </div>

                        {/* Quick Links */}
                        <div className="r2">
                            <h1 className="text-2xl font-extrabold text-black">Quick Links</h1>
                            <ul className='text-black text-md'>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Undergraduation course</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Masters/Phd course</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Shorts courses</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Scholarships</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Events and talks</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>About Maitri</a></li>
                            </ul>
                        </div>

                        {/* Countries */}
                        <div className="r3 sm:pl-0 lg:pl-10">
                            <h1 className="text-2xl font-extrabold text-black">Countries</h1>
                            <ul className='text-black text-sm'>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Italy</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>France</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Germany</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Ireland</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Spain</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>USA</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Canada</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Ukraine</a></li>
                            </ul>
                        </div>

                        {/* Courses */}
                        <div className="r4">
                            <h1 className="text-2xl font-extrabold text-black">Courses</h1>
                            <ul className='text-black text-sm'>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Portfolio preparation</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Italian language course</a></li>
                            </ul>
                        </div>

                        {/* Career Areas */}
                        <div className="r5">
                            <h1 className="text-2xl font-extrabold text-black">Career Areas</h1>
                            <ul className='text-black text-md'>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Arts, fashion & design</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Business & Management</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Film, Animation, Media & Acting</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Hospitality & Culinary Arts</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Events and talks</a></li>
                                <li><a href="#" className='dot-link hover:text-gray-300'>Engineering, Humanities & Social science</a></li>
                            </ul>
                        </div>
                    </div>

                    {/* Content for c-2 */}
                    <div className="c-2 grid grid-cols-1 lg:grid-cols-2 gap-6 text-center lg:text-left">
                        {/* Subscribe */}
                        <div className="search flex flex-col items-center lg:items-start">
                            <h1 className='text-2xl font-extrabold text-black'>Subscribe to our Newsletter</h1>
                            <p className='text-black'>Receive updates about our programs and events,</p>
                            <p className='text-black'>career tips and study abroad experiences.</p>
                            <div className="inp flex flex-col sm:flex-row items-center gap-2 mt-2 w-full sm:w-auto">
                                <input type="email" placeholder="Enter your email" className="p-2 rounded w-full sm:w-auto" />
                                <button className="p-2 bg-orange-500">
                                    <img src="data:image/svg+xml;base64," alt="" />
                                </button>
                            </div>
                        </div>

                        {/* Social Media */}
                        {/* Social Media */}
<div className="social flex flex-col items-center space-y-4 lg:items-start">
    <div>
        <img className="w-[200px] sm:w-[250px] md:w-[300px] h-auto" src="/maitri.png" alt="Logo" />
    </div>
    <div className="icon flex flex-wrap justify-center sm:justify-start gap-4">
        {/* Facebook */}
        <a href="#" className="duration-200">
            <img
                className="w-10 h-10 sm:w-12 sm:h-12 transition-transform duration-300 hover:scale-110"
                src="data:image/svg+xml;base64,"
                alt="Facebook"
            />
        </a>

        {/* Instagram */}
        <a href="#" className="duration-200">
            <img
                className="w-10 h-10 sm:w-12 sm:h-12 transition-transform duration-300 hover:scale-110"
                src="data:image/svg+xml;base64,"
                alt="Instagram"
            />
        </a>

        {/* Twitter */}
        <a href="#" className="duration-200">
            <img
                className="w-10 h-10 sm:w-12 sm:h-12 transition-transform duration-300 hover:scale-110"
                src="data:image/svg+xml;base64,"
                alt="youtube"
            />
        </a>
    </div>
</div>

                    </div>
                </div>
            </div>
        </div>
    );
}

export default Footer;
