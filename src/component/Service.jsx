import React from 'react';

const Service = () => {
  const services = [
    {
      title: 'Airport Pickup',
      desc: 'Seamless airport pickup upon arrival.',
      icon: 'data:image/svg+xml;base64,',
    },
    {
      title: 'Accommodation',
      desc: 'Help finding a comfortable stay.',
      icon: 'data:image/svg+xml;base64,',
    },
    {
      title: 'SIM Card Setup',
      desc: 'Get connected with local mobile service.',
      icon: 'data:image/svg+xml;base64,',
    },
    {
      title: 'Bank Account',
      desc: 'Assistance in opening a local bank account.',
      icon: 'data:image/svg+xml;base64,',
    },
    {
      title: 'City Orientation',
      desc: 'Get to know the local area and transport.',
      icon: 'data:image/svg+xml;base64,',
    },
    {
      title: 'Emergency Help',
      desc: '24/7 student support in case of emergency.',
      icon: 'data:image/svg+xml;base64,',
    },
  ];

  const topRow = services.slice(0, 4);
  const bottomRow = services.slice(4);

  return (
<div className='mt-0 mb-10 px-4'>
  {/* Header Section */}
  <div className='text-center mb-6'>
    <h1 className='text-4xl md:text-6xl font-extrabold'>Our Services</h1>
    <p className='text-base md:text-md mt-2'>
      HERE ARE THE SERVICES YOU CAN AVAIL FOR A SMALL FEE TO ENSURE A HASSLE-FREE STAY IN THIS NEW COUNTRY
    </p>
  </div>

  {/* Grid of 3 Cards per Row */}
  <div className='max-w-screen-xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8'>
    {services.map((service, index) => (
      <ServiceCard key={index} service={service} />
    ))}
  </div>
</div>


  );
};

const ServiceCard = ({ service }) => (
  <div className='group flex flex-col items-center justify-center p-4 border-2 w-56 transition-all duration-300 hover:bg-[#D4AF37] hover:text-white'>
    <img className='w-14 h-14 mb-2' src={service.icon} alt={`${service.title} icon`} />
    <h1 className='text-xl font-bold mb-1 group-hover:text-white'>{service.title}</h1>
    <p className='text-sm font-semibold text-center mb-3 group-hover:text-white'>{service.desc}</p>
    <button className='bg-[#EA8B07] text-white py-1 px-4 rounded hover:bg-white hover:text-[#EA8B07] transition-all'>
      Know more
    </button>
  </div>
);

export default Service;
